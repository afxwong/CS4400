...
create or replace view practiceQuery310 as
select fsid, remaining, usage_rate
from fund_source
where remaining < 7000 or (remaining / usage_rate) < 9;

create or replace view practiceQuery350 as
select pnumber, remote_access, (frequency * cost) as 'monthly_cost'
from maintenance_types
where pnumber in (select pnumber from maintenance);

create or replace view practiceQuery515 as
select dname, bcode, balance, remaining, usage_rate
from (select dname, bcode, balance, fsid from budget, department where budget.dnumber = department.dnumber) as temp
left join
fund_source as F
on temp.fsid = F.fsid;

create or replace view practiceQuery783 as
select pname, pnumber, plocation, dnum
from project as P, department as D
where P.dnum = D.dnumber and P.pnumber in
(select pnumber from maintenance where pnumber in (select pnumber from maintenance_types group by pnumber having count(*) < 3));
